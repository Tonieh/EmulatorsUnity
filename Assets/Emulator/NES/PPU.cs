...
public class PPU
{
	...
	public uint [] offscreenBuffer;

	public uint [] backgroundBuffer;
	public uint [] spriteBuffer;

	public uint [] currentScreen;
	
	//FIXME: should this be here?
	public Video myVideo;
	
	public static uint [] Nes_Palette =
	{
					...
					if ((pixelColor % 4) != 0)
					{
						if (vScrollSide == 0)
						{
							offscreenBuffer[(currentScanline * 256) + (8 * currentTileColumn) - scrollV + i] = 
								Nes_Palette[(0x3f & nameTables[0x1f00 + pixelColor])];
							backgroundBuffer[(currentScanline * 256) + (8 * currentTileColumn) - scrollV + i] = 
								Nes_Palette[(0x3f & nameTables[0x1f00 + pixelColor])];
						...
						else
						{
							if (((8 * currentTileColumn) + (256-scrollV) + i) < 256)
							{
								offscreenBuffer[(currentScanline * 256) + (8 * currentTileColumn) + (256-scrollV) + i] = 
									Nes_Palette[(0x3f & nameTables[0x1f00 + pixelColor])];
								backgroundBuffer[(currentScanline * 256) + (8 * currentTileColumn) + (256-scrollV) + i] = 
									Nes_Palette[(0x3f & nameTables[0x1f00 + pixelColor])];
						...
						if ((pixelColor % 4) != 0)
						{
							...
							{ 
								offscreenBuffer[(currentScanline * 256) + (spriteRam[i+3]) + j] = 
										Nes_Palette[(0x3f & nameTables[0x1f10 + pixelColor])];
								spriteBuffer[(currentScanline * 256) + (spriteRam[i+3]) + j] = 
										Nes_Palette[(0x3f & nameTables[0x1f10 + pixelColor])];
		...
		if (currentScanline < 234)
		{
			...
			if ((uint)nameTables[0x1f00] > 63)
			{
				for (i = 0; i < 256; i++)
				{
					...
					backgroundBuffer[(currentScanline * 256) + i] = 0;
					spriteBuffer[(currentScanline * 256) + i] = 16777216;
					sprite0Buffer[i] = 0;
				}
			}
			else
			{
				for (i = 0; i < 256; i++)
				{
					offscreenBuffer[(currentScanline * 256) + i] = Nes_Palette[(uint)nameTables[0x1f00]];
					backgroundBuffer[(currentScanline * 256) + i] = Nes_Palette[(uint)nameTables[0x1f00]];
					spriteBuffer[(currentScanline * 256) + i] = 16777216;
	...
	public PPU(NesEngine theEngine)
	{
		...
		offscreenBuffer = new uint[256 * 240];
		backgroundBuffer = new uint[256 * 240];
		spriteBuffer = new uint[256 * 240];
		currentScreen = new uint[256 * 240];
